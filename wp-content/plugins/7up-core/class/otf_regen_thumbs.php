	...
	function gambit_otf_regen_thumbs_media_downsize( $out, $id, $size ) {
		...
		if ( ! isset( $_gambit_otf_regen_thumbs_all_image_sizes ) ) {
			...
			foreach ( $interimSizes as $sizeName ) {
				if ( in_array( $sizeName, array( 'thumbnail', 'medium', 'large', 'medium_large', 'yith-woocompare-image', 'woocommerce_thumbnail', 'woocommerce_single', 'woocommerce_gallery_thumbnail', 'shop_catalog', 'shop_single', 'shop_thumbnail' ) ) ) {

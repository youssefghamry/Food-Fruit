...
<script type="text/javascript">
    jQuery(function () {
        jQuery('a.view-items').on('click', function (e) {
            ...
            if ( jQuery(this).text() == "<?php esc_html_e('Hide items', 'fruitshop'); ?>" ) {
                jQuery(this).text("<?php esc_html_e('View items', 'fruitshop'); ?>");
            } else {
                jQuery(this).text("<?php esc_html_e('Hide items', 'fruitshop'); ?>");
            }
...
<h2 class="title18 text-uppercase font-bold title-sales-orders"><?php esc_html_e( 'Orders', 'fruitshop' ); ?></h2>
...
<table class="table table-condensed table-vendor-sales-report">
    <thead>
    <tr>
        <th class="product-header"><?php esc_html_e( 'Order', 'fruitshop' ); ?></th>
        <?php if ( $can_view_address ) : ?>
            <th class="quantity-header"><?php esc_html_e( 'Shipping', 'fruitshop' ) ?></th>
        <?php endif; ?>
        <th class="commission-header"><?php esc_html_e( 'Total', 'fruitshop' ) ?></th>
        <th class="rate-header"><?php esc_html_e( 'Date', 'fruitshop' ) ?></th>
        <th class="rate-header"><?php esc_html_e( 'Links', 'fruitshop' ) ?></th>
    </thead>
    <tbody>
            ...
            <tr id="order-<?php echo $order_id; ?>" data-order-id="<?php echo $order_id; ?>">
                ...
                <td>
                    ...
                    $order_actions = array(
                        'view'		=> array(
                            'class' 	=> 'view-items',
                            'content'	=> esc_html__('View items', 'fruitshop'),
                        )
                    );
                    if ( $needs_shipping ) {
                        $order_actions['shipped'] = array(
                            'class' 	=> 'mark-shipped',
                            'content'	=> esc_html__('Mark shipped', 'fruitshop'),
                    ...
                    if ( $shipped ) {
                        $order_actions[ 'shipped' ] = array(
                            'class' 	=> 'mark-shipped',
                            'content'	=> esc_html__('Shipped', 'fruitshop'),
                            'url'		=> '#'
                        );
                    }
                    ...
                    if ( $providers && $needs_shipping && class_exists( 'WC_Shipment_Tracking' ) ) {
                        $order_actions['tracking'] = array(
                            ...
                            'content'	=> esc_html__( 'Tracking', 'fruitshop' )
                        );
                    }
    ...
    <?php else : ?>
        ...
        <tr>
            <td colspan="4"
                style="text-align:center;"><?php esc_html_e( 'You have no orders during this period.', 'fruitshop' ); ?></td>
        </tr>
